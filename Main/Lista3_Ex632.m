...
Phi1 = eye(size(M));
lambda = zeros(size(M, 1), 1); %não tenho certeza se é isso
lambda_old = lambda; %não tenho certeza se é isso
...
while true
    ...
    % Sweeping
    for i = 1:sizeM-1 %não tenho certeza se é isso
        for j = i+1:sizeM %não tenho certeza se é isso
            % Test for rotation
            epsilon_r = 10^(-2*m);
            ...
            %a expressão sign(a2) é usada para determinar o sinal de a2 para fins de cálculos adicionais
    ...
    % Update eigenvalues
    for i = 1:sizeM %não tenho certeza
        lambda(i) = K(i,i) / M(i,i);
    end
    
    % Test for convergence
    if max(abs(lambda - lambda_old)./lambda) <= epsilon %não tenho certeza do max
